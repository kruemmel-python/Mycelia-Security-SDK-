...
## Voraussetzungen
- **Java 21+ JDK** auf dem `PATH`. Das Build setzt `--release 21`, damit der generierte Bytecode exakt zum Paper-API-Target passt.
...
> Hinweis: Das Projekt nutzt `paper-api:1.21.11-R0.1-SNAPSHOT` als `compileOnly`-Abhängigkeit. Dadurch bleiben die Paper/Spigot-Klassen zur Kompilezeit verfügbar, werden aber nicht ins JAR gepackt (Paper/Spigot stellt sie zur Laufzeit bereit).
...
- **Toolchain/Version-Fehler mit JDK 21**: Das Build nutzt dein installiertes JDK (kein Gradle-Toolchain-Download). Stelle sicher, dass `java -version` mindestens 21 meldet.
