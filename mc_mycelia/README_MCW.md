...
- Häufige Stolperfallen beim Python-Skript:
  ...
  - Setze bei langen GPU-Initialisierungen den Timeout passend (Arg `--timeout` oder env `MYCELIA_DRIVER_TIMEOUT`, Default 90 s; in der Config kannst du `driver.timeoutSeconds` und optional `driver.warmupTimeoutSeconds` setzen).
